...
export function QuantitativeTerritoryMap({
  ...
  territoriesLabel = "territori rappresentati",
}: Props) {
  const mappable = rows
  ...
  return (
    <figure className="border border-black bg-white">
      ...
      <div className="overflow-x-auto p-3 sm:p-5">
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-auto min-w-[680px] w-full" role="img" aria-label={`${title}. ${mappable.length} ${territoriesLabel}.`}>
        ...
        <ul className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
          {[...mappable].sort((a, b) => b.value - a.value).map((row) => (
            <li key={row.id} className="flex justify-between gap-3 border-b border-neutral-200 pb-2">
              <span>{row.territoryLabel}</span><strong>{row.formattedValue}</strong>
            </li>
          ))}
        </ul>
      </div>
