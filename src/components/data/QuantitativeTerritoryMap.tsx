...
function italyProject(lon: number, lat: number) {
  ...
  const minLat = 35.0;
  const maxLat = 48.5;
  return { x: ((lon - minLon) / (maxLon - minLon)) * WIDTH, y: ((maxLat - lat) / (maxLat - minLat)) * HEIGHT };
}
  ...
  return (
    <figure className="border border-black bg-white">
      ...
      <div className="overflow-x-auto p-3 sm:p-5">
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="h-auto min-w-[680px] w-full" role="img" aria-label={`${title}. ${ariaTerritories(mappable.length)}`}>
          ...
          {mappable.map((row) => {
            ...
            return (
              <g key={row.id}>
                <circle cx={point.x} cy={point.y} r={radius} fill="black" fillOpacity="0.72" stroke="black" strokeWidth="1.5">
                  <title>{`${row.territoryLabel}: ${row.formattedValue}`}</title>
                </circle>
                <text x={point.x + radius + 5} y={point.y + 4} fontSize="12" fontWeight="600" fill="currentColor">{row.territoryLabel}</text>
              </g>
            );
          })}
        </svg>
      ...
      <div className="border-t border-black px-5 py-4">
        <ul className="grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-3">
          {[...mappable].sort((a, b) => b.value - a.value).map((row) => (
            <li key={row.id} className="flex justify-between gap-3 border-b border-neutral-200 pb-2"><span>{row.territoryLabel}</span><strong>{row.formattedValue}</strong></li>
          ))}
        </ul>
      </div>
    </figure>
