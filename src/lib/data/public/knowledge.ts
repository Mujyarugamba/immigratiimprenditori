...
export type KnowledgeNode = {
  id: string;
  kind: "country" | "indicator" | "sector" | "route";
  label: string;
  href: string;
};
...
export type KnowledgeEdge = {
  from: string;
  predicate: "observed_in" | "classified_in" | "origin_of" | "destination_of";
  to: string;
};
...
export async function getPublicKnowledgeSnapshot(): Promise<KnowledgeSnapshot> {
  ...
  const publicCountries = new Set(
    countrySummaries.filter((item) => item.hasEvidence).map((item) => item.country.code),
  );
  ...
  const indicatorById = new Map(explorer.indicators.map((item) => [item.id, item]));
  ...
  for (const value of explorer.values) {
    ...
    if (country && publicCountries.has(country.code)) {
      ...
      edges.set(key, { from: indicatorNodeId, predicate: "observed_in", to: countryNodeId });
    }
    ...
    if (value.business_sector_id != null) {
      ...
      if (sector) {
        ...
        edges.set(key, { from: indicatorNodeId, predicate: "classified_in", to: sectorNodeId });
      }
    }
  }
  ...
  return {
    nodes: Array.from(nodes.values()).sort((a, b) => a.label.localeCompare(b.label, "it")),
