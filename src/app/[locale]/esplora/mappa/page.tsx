...
export default async function LocalizedQuantitativeMapPage({params,searchParams}:Props){
  ...
  for(const value of indicatorValues){if(value.country_code&&value.country_label) categoryMap.set(value.country_code,localizedTerritoryLabel(locale,value.country_code,value.country_label));}
  ...
  const selectedCategory=requiresCategory?(categories.some(([code])=>code===query.categoria)?query.categoria:null):(categories[0]?.[0]??null);
  ...
  const mapRows=indicator?filtered.map((value)=>({id:value.id,territoryCode:value.territory_code!,territoryLabel:localizedTerritoryLabel(locale,value.territory_code,value.territory_label),value:Number(value.numeric_value),formattedValue:formatExplorerValue(Number(value.numeric_value),indicator.unit_code,locale)})):[];
  ...
  return <main id="contenuto" className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:py-16">
    ...
    <form method="get" className="mt-8 grid gap-5 border border-black p-5 md:grid-cols-4">
      <label className="flex flex-col gap-2 text-sm font-semibold text-black">{m.indicator}<select name="indicatore" defaultValue={indicator?.slug??""} className="border border-neutral-400 bg-white px-3 py-2.5 font-normal">{snapshot.indicators.map((item)=><option key={item.id} value={item.slug}>{indicatorTranslation(locale,item.slug)?.title??item.title}</option>)}</select></label>
      <label className="flex flex-col gap-2 text-sm font-semibold text-black">{m.year}<select name="anno" defaultValue={selectedYear??""} className="border border-neutral-400 bg-white px-3 py-2.5 font-normal">{years.map((year)=><option key={year} value={year}>{year}</option>)}</select></label>
      <label className="flex flex-col gap-2 text-sm font-semibold text-black">{m.category}<select name="categoria" defaultValue={selectedCategory??""} className="border border-neutral-400 bg-white px-3 py-2.5 font-normal" disabled={!requiresCategory}>{requiresCategory?<option value="">{m.select}</option>:null}{categories.map(([code,label])=><option key={code} value={code}>{label}</option>)}</select></label>
      <div className="flex items-end gap-3"><button type="submit" className="border border-black bg-black px-5 py-2.5 text-sm font-semibold text-white">{m.update}</button><Link href={`/${locale}/esplora/dati`} className="px-2 py-2.5 text-sm underline underline-offset-4">{m.table}</Link></div>
    </form>
    {requiresCategory&&!selectedCategory?<section className="mt-8 border border-black p-6" aria-live="polite"><p className="max-w-3xl text-base leading-7 text-neutral-700">{m.choose}</p></section>:mapRows.length>0&&indicator?<section className="mt-8"><QuantitativeTerritoryMap title={mapTitle} rows={mapRows} eyebrow={m.eyebrow} description={m.mapDescription} ariaTerritories={m.aria}/></section>:<section className="mt-8 border border-black p-6" aria-live="polite"><p className="max-w-3xl text-base leading-7 text-neutral-700">{m.none}</p></section>}
    <section className="mt-10 border-t border-black pt-8"><h2 className="text-xl font-semibold text-black">{m.interpretation}</h2><p className="mt-4 max-w-4xl text-sm leading-7 text-neutral-700">{m.interpretationText}</p>{indicator?<Link href={`/${locale}/osservatorio/${indicator.slug}`} className="mt-5 inline-block text-sm font-semibold underline underline-offset-4">{m.record} →</Link>:null}</section>
  </main>;
}
