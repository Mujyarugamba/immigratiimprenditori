        ...
        <table className="min-w-full border-collapse text-left text-sm">
          ...
          <tbody>
            {filtered.map((value) => {
              ...
              const quality = value.quality_code === "official" ? m.official : value.quality_code.replaceAll("_", " ");
              return <tr key={value.id} className="border-b border-neutral-300 last:border-b-0"><td className="px-4 py-3 align-top"><Link href={`/${locale}/osservatorio/${indicator.slug}`} className="font-semibold underline underline-offset-4">{translated?.title ?? indicator.title}</Link></td><td className="px-4 py-3 align-top">{localizedTerritoryLabel(locale, value.territory_code, value.territory_label)}</td><td className="px-4 py-3 align-top">{localizedTerritoryLabel(locale, value.country_code, value.country_label)}</td><td className="px-4 py-3 align-top">{value.period_start === value.period_end ? value.period_start : `${value.period_start} → ${value.period_end}`}</td><td className="px-4 py-3 text-right align-top font-semibold">{formatExplorerValue(Number(value.numeric_value), indicator.unit_code, locale)}</td><td className="px-4 py-3 align-top">{quality}</td></tr>;
            })}
