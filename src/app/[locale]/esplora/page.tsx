...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: m.exploreTitle, description: m.exploreIntro, alternates: { canonical: `/${locale}/esplora` } };
}
...
export default async function LocalizedExplorePage({ params }: Props) {
  ...
  const modules = [
    { title: m.dataExplorer, text: texts[0], href: "/esplora/dati" },
    { title: m.territories, text: texts[1], href: "/esplora/territori" },
    { title: m.sectors, text: texts[2], href: "/esplora/settori" },
    { title: m.sources, text: texts[3], href: "/fonti" },
    { title: m.glossary, text: texts[4], href: "/glossario" },
    { title: m.openData, text: texts[5], href: "/open-data" },
  ];
  ...
  return (
    <main id="contenuto" className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:py-16">
      ...
      <section className="mt-8 grid gap-px border border-black bg-black sm:grid-cols-4">
        <div className="bg-white p-5"><strong className="text-3xl">{snapshot.indicators.length}</strong><p className="mt-2 text-xs text-neutral-500">Indicators</p></div>
        <div className="bg-white p-5"><strong className="text-3xl">{snapshot.values.length}</strong><p className="mt-2 text-xs text-neutral-500">Data points</p></div>
        <div className="bg-white p-5"><strong className="text-3xl">{snapshot.territories.length}</strong><p className="mt-2 text-xs text-neutral-500">Territories</p></div>
        <div className="bg-white p-5"><strong className="text-3xl">{snapshot.sectors.length}</strong><p className="mt-2 text-xs text-neutral-500">Sectors</p></div>
      </section>
      ...
      <section className="mt-8 grid gap-px border border-black bg-black md:grid-cols-3">
        {modules.map((module) => (
          <article key={module.href} className="flex min-h-52 flex-col bg-white p-6">
            ...
            <Link href={module.href} className="mt-5 text-sm font-semibold underline underline-offset-4">Open →</Link>
          </article>
        ))}
      </section>
    </main>
