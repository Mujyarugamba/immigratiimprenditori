...
const labels = {
  en: { back: "Back to Observatory", purpose: "Purpose", method: "Methodology", values: "Published values", source: "Source" },
  fr: { back: "Retour à l'Observatoire", purpose: "Objectif", method: "Méthodologie", values: "Valeurs publiées", source: "Source" },
  es: { back: "Volver al Observatorio", purpose: "Objetivo", method: "Metodología", values: "Valores publicados", source: "Fuente" },
  de: { back: "Zurück zum Observatorium", purpose: "Zweck", method: "Methodik", values: "Veröffentlichte Werte", source: "Quelle" },
  ar: { back: "العودة إلى المرصد", purpose: "الهدف", method: "المنهجية", values: "القيم المنشورة", source: "المصدر" },
  zh: { back: "返回观察站", purpose: "目的", method: "方法", values: "已发布数据", source: "来源" },
} as const;
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/${locale}/osservatorio/${indicator.slug}` },
  };
}
  ...
  return (
    <main id="contenuto" className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:py-16">
      ...
      <header className="mt-6 max-w-4xl border-b border-black pb-8">
        ...
        <h1 className="mt-3 text-4xl font-semibold tracking-tight text-black sm:text-5xl">{indicator.title}</h1>
        <p className="mt-5 text-lg leading-8 text-neutral-700">{indicator.description}</p>
        <p className="mt-3 text-sm leading-6 text-neutral-600">{core.originalLanguageNotice}</p>
      </header>

      <section className="mt-8 grid gap-8 lg:grid-cols-2">
        <div>
          <h2 className="text-xl font-semibold text-black">{l.purpose}</h2>
          <p className="mt-3 text-sm leading-7 text-neutral-700">{indicator.purpose_text}</p>
        </div>
        <div>
          <h2 className="text-xl font-semibold text-black">{l.method}</h2>
          <p className="mt-3 text-sm leading-7 text-neutral-700">{indicator.methodology_summary}</p>
        </div>
      </section>
      ...
      <section className="mt-10 border-t border-black pt-8">
        ...
        <div className="mt-5 overflow-x-auto border border-black">
          <table className="min-w-full text-left text-sm">
            <thead className="bg-neutral-100"><tr><th className="border-b border-black px-4 py-3">Territory</th><th className="border-b border-black px-4 py-3">Category</th><th className="border-b border-black px-4 py-3">Period</th><th className="border-b border-black px-4 py-3 text-right">Value</th><th className="border-b border-black px-4 py-3">{l.source}</th></tr></thead>
            <tbody>
              {indicator.values.map((value) => (
                <tr key={value.id} className="border-b border-neutral-300 last:border-b-0">
                  ...
                  <td className="px-4 py-3 text-right font-semibold">{formatExplorerValue(Number(value.numeric_value), indicator.unit_code)}</td>
