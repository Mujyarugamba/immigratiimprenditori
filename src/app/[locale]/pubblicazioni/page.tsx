...
function kindLabel(locale:Exclude<Parameters<typeof isPlatformLocale>[0],never>,kind:string|null,typeCode:string){
 const m=(text as typeof text)[locale as keyof typeof text];
 ...
 if(kind&&kind in m.kinds)return m.kinds[kind as keyof typeof m.kinds];
...
export async function generateMetadata({params}:Props):Promise<Metadata>{
 ...
 const m=text[locale];
 return {title:m.title,description:m.description,alternates:{canonical:`/${locale}/pubblicazioni`,languages:languageAlternates("/pubblicazioni")},...pageSocialMetadata({title:m.title,description:m.description,pathname:`/${locale}/pubblicazioni`})};
}
...
export default async function LocalizedPublicationsPage({params}:Props){
 ...
 const translated=await presentLocalizedContentByHrefs(publications.map((item)=>`/contenuti/${item.slug}`),locale);
 return <main id="contenuto" className="preview-hub-page">
   <section className="preview-hub-hero publications">
     ...
     <div className="preview-hub-inner"><p className="preview-hub-kicker">{m.kicker}</p><h1>{m.title}</h1><p className="hub-intro">{m.intro}</p></div>
   </section>
   <div className="preview-hub-body">
     <div className="preview-section-head"><div><p className="eyebrow">{m.library}</p><h2>{m.section}</h2></div><div className="flex flex-wrap gap-4"><Link href={`/${locale}/contenuti`}>{m.analysis} {arrow}</Link><Link href={`/${locale}/fonti`}>{m.sources} {arrow}</Link><Link href={`/${locale}/dati-e-fonti`}>{m.method} {arrow}</Link></div></div>
     {publications.length>0?<div className="preview-index-grid">{publications.map((item)=>{const p=translated.get(item.slug);const title=p?.title??item.title;const abstract=p?.abstract??item.abstract;return <article key={item.id} className="preview-index-card"><p className="index-meta">{kindLabel(locale,item.report_kind,item.type_code)}{item.source_publication_year?` · ${item.source_publication_year}`:""}</p><h2><Link href={`/${locale}/contenuti/${item.slug}`}><OriginalLanguageText as="span" languageCode={p?.displayLanguageCode}>{title}</OriginalLanguageText></Link></h2>{item.authors.length>0?<p className="font-semibold">{item.authors.join(", ")}</p>:null}{item.publisher_name?<p>{item.publisher_name}</p>:null}{abstract?<OriginalLanguageText languageCode={p?.displayLanguageCode}>{abstract}</OriginalLanguageText>:null}<div className="index-footer flex flex-wrap gap-4"><Link href={`/${locale}/contenuti/${item.slug}`}>{m.open} {arrow}</Link>{item.document_url?<a href={item.document_url} target="_blank" rel="noopener noreferrer">{m.original} ↗</a>:null}</div></article>})}</div>:<p>{m.empty}</p>}
   </div>
 </main>;
}
