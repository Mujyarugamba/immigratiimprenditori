...
export function DeleteAccountForm({
  ...
  return (
    ...
    >
      ...
      <p className="text-ink mt-3 text-sm">{SELF_DELETE_USER_COPY.summary}</p>
      {willOpenReassignment ? (
      ...
      <p className="text-ink mt-4 text-sm font-medium">
        Inserisci la password e scrivi {SELF_DELETE_CONFIRM_PHRASE}.
      </p>
      ...
      <form action={action} className="mt-4 flex flex-col gap-4">
        ...
        !state.fieldErrors?.confirm_phrase ? (
        ...
        >
          {pending ? "Cancellazione in corso…" : "Cancella account"}
        </Button>
      </form>
    </section>
  );
