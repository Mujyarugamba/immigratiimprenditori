...
**Limite dichiarato su "Contatti".** La Data Specification prevede contatti multipli con visibilità scelta canale per canale (pubblico / utenti registrati). La struttura attuale (`website`, `phone` come singole colonne, sempre soggette alla visibilità complessiva del profilo) non implementa questa granularità. Si tratta di una semplificazione consapevole per la prima versione (principio di non sovraprogettazione, §2.8): l'estensione a contatti multipli con visibilità indipendente è rinviata (§15) e non richiede, quando servirà, alcuna modifica distruttiva (si aggiungerebbe una tabella `profile_contacts` dipendente, senza toccare queste colonne).
